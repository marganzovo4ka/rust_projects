...
pub fn div_voltage() {
    ...
    if u_in / 2.0 < u_out{
        ...
        if flag {
            ...
            let mut value_less:(f32, f32, f32) = (0.0, 0.0, 100000.0);
            let mut value_biger:(f32, f32, f32) = (0.0,0.0, -100000.0);
            let mut less_ratio:f32 = 100000.0;
            let mut biger_ratio:f32 = -100000.0;
            println!("No exact matches were found");
            for i in 0..e96.len()/2{
                let r1 = e96[i];
                let mut r2 = (u_in/(u_in - u_out) - 1.0)*r1;
                let mut less_index:usize = 1000;
                let mut biger_index:usize = 1000;

                if find_approx(&e96, r2, &mut biger_index, &mut less_index){
                    if less_index < 192{
                        r2 = e96[less_index];
                        let u_approx_less = u_in *r2/(r1+r2);
                        if u_out - u_approx_less < less_ratio{
                            value_less = (r1,r2, u_approx_less);
                            less_ratio = u_out - u_approx_less;
                        }
                    }
                    if biger_index < 192{
                        r2 = e96[biger_index];
                        let u_approx_biger = u_in *r2/(r1+r2);
                        if u_out - u_approx_biger > biger_ratio{
                            value_biger = (r1,r2, u_approx_biger);
                            biger_ratio = u_out - u_approx_biger;
                        }
                    }
                }
            }
            result(value_less, value_biger, u_in, u_out);
         }
     }
    else{
...
fn find_approx(arr: &Vec<f32>, find:f32, less_index: &mut usize, biger_index: &mut usize) -> bool{
    ...
    while l+1 != r{
        ...
        if arr[l]> find && arr[r] > find{
            *less_index = l;
            *biger_index = l - 1 ;
        }
        else if arr[l] < find && arr[r] < find{
            *biger_index = r;
            *less_index = r + 1;
        }

        if (arr[l] - find).abs() < 10.0{
            if arr[l] > find{
            *less_index = l;
            }
            else{
        ...
        if *biger_index < 192 || *less_index < 192{
            return true;
        }
        else {
            return false;
...
fn result(value_less: (f32,f32,f32), value_biger: (f32,f32,f32), u_in: f32, u_out: f32){
    if (value_less.0 != 0.0 && value_biger.0 != 0.0) || value_less.0 == 0.00 || value_biger.0 == 0.0{
...
Output Voltage: {} 
Avsolute mistake: {} 
Relative mistake: {} % \n", value_less.0, value_less.1, value_less.2, value_less.2 - u_out, (value_less.2 - u_out)/u_out * 100.0);

println!(" Smaller ratio \n
devider information:
Input voltage: {u_in} V 
R1: {} Ohms 
R2: {} Ohms 
...
Output Voltage: {} 
Avsolute mistake: {} 
Relative mistake: {} %", value_biger.0, value_biger.1, value_biger.2, value_biger.2 - u_out, (value_biger.2 - u_out)/u_out * 100.0)   
    }    
}
